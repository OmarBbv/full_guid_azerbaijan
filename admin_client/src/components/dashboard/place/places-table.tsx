...
import { PlaceStatus, PlaceType } from '@/types/restaurant';
...
const typeColor: Record<PlaceType, 'default' | 'primary' | 'secondary' | 'info' | 'warning' | 'error' | 'success'> = {
  [PlaceType.RESTAURANT]: 'warning',
  [PlaceType.HOTEL]: 'primary',
  [PlaceType.HOSTEL]: 'secondary',
  [PlaceType.LANDMARK]: 'info',
  [PlaceType.NATURE]: 'success',
  [PlaceType.ENTERTAINMENT]: 'secondary',
  [PlaceType.MUSEUM]: 'primary',
  [PlaceType.OTHER]: 'default',
};
...
export function PlacesTable({
  ...
  onDelete,
  onEdit,
}: PlacesTableProps): React.JSX.Element {
  ...
  if (error) {
    return (
      <Alert severity="error">Məkanlar yüklənərkən xəta baş verdi: {error.message}</Alert>
    );
  }
              ...
              rows.map((row) => {
                ...
                return (
                  <TableRow hover key={row.id} selected={isSelected}>
                    ...
                    <TableCell>
                      <Chip label={row.type} color={typeColor[row.type]} size="small" variant="outlined" />
                    </TableCell>
                    <TableCell>
                      <Chip label={row.status} color={statusColor[row.status]} size="small" />
                    </TableCell>
                    <TableCell>{row.average_rating != null ? Number(row.average_rating).toFixed(1) : '—'} ⭐</TableCell>
