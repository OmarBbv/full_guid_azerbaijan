...
export function VenueEditForm({ id }: VenueEditFormProps): React.JSX.Element {
  const router = useRouter();
  const [isUploading, setIsUploading] = React.useState(false);
  const [previewUrl, setPreviewUrl] = React.useState<string | null>(null);

  const { data: venue, isLoading: isLoadingVenue, isError: isLoadError } = useVenueAdmin(id);
  const { mutate: updateVenue, isPending: isUpdating, isError: isUpdateError, error: updateError } = useUpdateVenue(id);
  const { data: allCategories = [], isLoading: isLoadingCategories } = useCategories();

  const { register, handleSubmit, control, setValue, watch, reset, formState: { errors } } = useForm<VenueFormValues>({
  ...
  React.useEffect(() => {
    if (venue) {
      reset({
        name: venue.name || '',
        language: (venue.language as any) || 'az',
  ...
  if (isLoadError) {
    return (
      <Alert severity="error">
        Məkan məlumatlarını yükləyərkən xəta baş verdi.
      </Alert>
    );
  }
    ...
    <form onSubmit={handleSubmit(onSubmit as any)} noValidate>
      <Stack spacing={4}>
        <Stack direction="row" spacing={2} alignItems="center">
          ...
          <Typography variant="h4">Məkanı Redaktə Et</Typography>
        </Stack>
                  ...
                  render={({ field }) => (
                    <FormControl fullWidth>
                      ...
                      <Select
                        ...
                        onChange={(e) => {
                          field.onChange(e);
                          // Reset category when language changes if it's not the original one
                          if (venue?.language !== e.target.value) {
        ...
        <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2 }}>
          ...
          >
            {isUpdating ? 'Saxlanılır...' : 'Məkanı Yenilə'}
          </Button>
        </Box>
      </Stack>
    </form>
