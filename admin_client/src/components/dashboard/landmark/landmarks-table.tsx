...
export function LandmarksTable({
  ...
  onDelete,
  onEdit,
}: LandmarksTableProps): React.JSX.Element {
  ...
  if (error) {
    return (
      <Alert severity="error">
        Landmarklər yüklənərkən xəta baş verdi: {error.message}
      </Alert>
    );
  }
              ...
              rows.map((row) => {
                ...
                return (
                  <TableRow hover key={row.id} selected={isSelected}>
                    ...
                    <TableCell>
                      <Chip label={status} color={statusColor[status] || 'warning'} size="small" />
                    </TableCell>
                    <TableCell>
