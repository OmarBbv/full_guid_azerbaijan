...
export default function Page(): React.JSX.Element {
  ...
  const [pages, setPages] = React.useState<AboutPage[]>([]);
  ...
  const fetchPages = React.useCallback(async (lang?: string) => {
    ...
    try {
      ...
      const data = await aboutPageService.getAll(params);
      setPages(data);
    } catch (error) {
      toast.error('Səhifələr yüklənərkən xəta baş verdi');
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await aboutPageService.remove(selectedId);
    ...
    <Stack spacing={3}>
      <Stack direction="row" spacing={3} alignItems="center">
        <Stack spacing={0.5} sx={{ flex: '1 1 auto' }}>
          <Typography variant="h4">Haqqımızda Səhifələri</Typography>
        ...
        <Button
          ...
          variant="contained"
          onClick={() => router.push(paths.dashboard.aboutCreate)}
        >
          Yeni Səhifə
        </Button>
      </Stack>
      ...
      <AboutTable
        ...
        onEdit={(id) => router.push(paths.dashboard.aboutEdit(id))}
      ...
      <DeleteConfirmationDialog
        ...
        title="Səhifəni sil"
        description="Bu səhifəni silmək istədiyinizə əminsiniz? Bu əməliyyat geri qaytarıla bilməz."
      />
    </Stack>
  );
}
